...
int main(int argc, char **argv) {
        ...
        PGresult   *res;
        PGnotify   *notify;
        char db_name[] = "sherbata_irina";
        char user_name[] = "sherbata_irina";
        ...
        res = PQexec(conn, "LISTEN sherbata_irina");
        if(PQresultStatus(res) != PGRES_COMMAND_OK) {
                fprintf(stderr, "LISTEN command failed, bro: %s", PQerrorMessage(conn));
                PQclear(res);
                PQfinish(conn);
                exit(1);
        }
